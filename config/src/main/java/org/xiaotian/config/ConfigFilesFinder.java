...
public class ConfigFilesFinder {
	...
	/**
	 * 要查找的配置文件的名称，如 config.xml 或者 beans.xml
	 */
	...
	public ConfigFilesFinder(ArrayList<String> _arConfigFileRootPaths, String _sConfigXmlFile) {
	...
	public ConfigFiles getConfigFiles() throws WCMException {
		if (m_oConfigFiles == null) {
			...
			for (int i = 0; i < m_pConfigFileRootPaths.size(); i++) {
				String sConfigFileRootPath = (String) m_pConfigFileRootPaths.get(i);

				if (s_logger.isDebugEnabled())
					s_logger.debug("begin to load config files from[" + sConfigFileRootPath + "]...");
			...
			if (s_logger.isDebugEnabled())
				s_logger.debug("the files have been loaded.");
		}
	...
	private void lookupCfgFiles(File _file) {
		if (_file.isFile()) {
			...
			String sMapping = CMyFile.extractFilePath(_file.getPath()) + ConfigConstants.NAME_FILE_MAPPING;
			File fMapping = CMyFile.fileExists(sMapping) ? new File(sMapping) : null;
			...
			ConfigFile configFile = (ConfigFile) m_mapAlreadyDoneWithFileNames.get(sConfigFileNameExcludeProjectPath);
			if (configFile != null) {
			...
			m_mapAlreadyDoneWithFileNames.put(sConfigFileNameExcludeProjectPath, configFile);

			if (s_logger.isDebugEnabled()) {
				s_logger.debug("load xml file[" + _file.getPath() + "]");
			}
		...
		// wenyh@2007-6-19 9:22:43 add comment:listFiles may be return null.
		if (dirs == null) {
			s_logger.warn("May be a IOException,find an invalid dir:" + _file.getAbsolutePath());
			return;
		}
	...
	private String extractConfigFileNameExcludeProjectPath(String _sAbsolutFileName) {
		String sConfigFilePathFlag = File.separatorChar + TRSConfigServer.getAppRootPath() + File.separatorChar;
